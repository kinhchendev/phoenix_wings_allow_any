...
import 'package:phoenix_wings_allow_any/src/phoenix_push.dart';
import 'package:phoenix_wings_allow_any/src/phoenix_socket.dart';
...
class PhoenixChannel {
  ...
  /// Pushes an any (dynamic) message to the server
  PhoenixPush? pushAny({String? event, dynamic payloadAny}) {
    if (!_joinedOnce) {
      throw ("Tried to push event before joining channel");
    }
    final pushEvent = new PhoenixPush(this, event, null, timeout);
    final pushEventAny = new PhoenixPush.fromPush(pushEvent, payloadAny);
    if (canPush) {
      pushEventAny.sendAny();
    } else {
      pushEventAny.startTimeout();
      _pushBuffer.add(pushEventAny);
    }
    return pushEventAny;
  }
